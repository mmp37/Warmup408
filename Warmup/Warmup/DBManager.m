...
-(BOOL)createDB{
    ...
    databasePath = [[NSString alloc] initWithString:
                    [docsDir stringByAppendingPathComponent: @"users.db"]];
    BOOL isSuccess = YES;
    ...
    if ([filemgr fileExistsAtPath: databasePath ] == NO)
    {
        ...
        if (sqlite3_open(dbpath, &database) == SQLITE_OK)
        {
            char *errMsg;
            const char *sql_stmt =
            "create table if not exists userPass (name text, password text)";
...
- (BOOL) saveData:(NSString*)name
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK)
    {
        NSString *insertSQL = [NSString stringWithFormat:@"insert into userPass (name, password) values (\"%@\", \"%@\")",
                                name, password];
